...
from src.coref.common import ControllerBase, NeuralCoreferencePairScorer
from src.coref.data import read_corpus, Document
from src.coref.utils import split_into_sets, fixed_split, KFoldStateCache
...
class ContextualControllerBERT(ControllerBase):
    def __init__(self,
        ...
        self.embedder = AutoModel.from_pretrained(pretrained_model_name_or_path,
    ...
    def pred(self, input_seq):
        ...
        input_ids = self.tokenizer.encode(input_seq[0])
        tokens = self.tokenizer.convert_ids_to_tokens(input_ids)
        tokenized_doc, mapping = [], {}
        idx_tokenized = 0
        for idx_sent, curr_sent in enumerate(input_seq):
            for idx_inside_sent, curr_token in enumerate(curr_sent.split(" ")):
                tokenized_token = self.tokenizer.tokenize(curr_token)
                tokenized_doc.extend(tokenized_token)
                mapping[(idx_sent, idx_inside_sent)] = list(range(idx_tokenized, idx_tokenized + len(tokenized_token)))
                idx_tokenized += len(tokenized_token)
        logging.info(tokenized_doc)
        logging.info(mapping)
        logging.info("The input has a total of {} tokens.".format(len(tokenized_doc)))
        tokens = self.tokenizer.convert_tokens_to_ids(tokenized_doc)
        ...
        for idx_segment in range(num_total_segments):
            ...
            curr_seg = self.tokenizer.prepare_for_model(ids=tokens[s_seg: e_seg],
        ...
        assert len(segments["input_ids"]) == len(segments["token_type_ids"])
        output = self.embedder(segments["input_ids"], token_type_ids=segments["token_type_ids"])
        scores = output["pooler_output"]
        maxes = torch.argmax(scores, dim=1).tolist()
        logging.info(segments["input_ids"].size())
        logging.info(scores.size())
        logging.info(len(maxes))
        inputs = segments["input_ids"].tolist()[0]
        test = self.tokenizer.convert_ids_to_tokens(inputs)
        test2 = self.tokenizer.convert_ids_to_tokens(maxes)
        logging.info(test)
        logging.info(test2)
