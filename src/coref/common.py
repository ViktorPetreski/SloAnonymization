...
import src.coref.metrics as metrics
import torch
...
class ControllerBase:
    def __init__(self, learning_rate, dataset_name, early_stopping_rounds=3, model_name=None):
    ...
    def train(self, epochs, train_docs, dev_docs):
        ...
        for idx_epoch in range(epochs):
            ...
            for curr_doc in dev_docs:
                _, (doc_loss, n_examples), _ = self._train_doc(curr_doc, eval_mode=True)
        ...
        logging.info(f"Training complete: took {(time.time() - t_start) / 60:.2f} min")
